...
class NoticiaController
{
    ...
    public function postInsert()
    {
        ...
        $Imag_Noti = $_FILES['imag_noti']['name'];
        $ruta = "img/$Imag_Noti";

        move_uploaded_file($_FILES['imag_noti']['tmp_name'], $ruta);

        $id = $obj->autoincrement("t_noticia", "cod_noti");

        $sql = "INSERT INTO t_noticia VALUES($id, $Usu_Id, $Cod_Tipo_Not, 0, '$Desc_Noti', '$Tit_Noti', '$Fech_Noti', '$ruta')";
    ...
    public function postUpdate()
    {
        ...
        if (isset($_FILES['imag_noti']['name'])) {
            $Imag_Noti = $_FILES['imag_noti']['name'];
            $ruta = "img/$Imag_Noti";
            move_uploaded_file($_FILES['imag_noti']['tmp_name'], $ruta);
            ...
            $sql = "UPDATE t_noticia SET cod_tipo_not = '$Cod_Tipo_Not', tit_noti = '$Tit_Noti', desc_noti = '$Desc_Noti', imag_noti = '$ruta' WHERE cod_noti = $Cod_Noti";
        }
        $ejecutar = $obj->update($sql);
